...
DBuser = "postgres"
DBpwd = "postgres"
TempTableName = 'UnloggedCensusData'
TableName = 'CensusData'
Datafile = "AL2015_1.csv"  # name of the data file to be loaded
...
# convert list of data rows into list of SQL 'INSERT INTO ...' commands
def getSQLcmndsUnloggedTable(rowlist):
	cmdlist = []
	for row in rowlist:
		valstr = row2vals(row)
...
# Add indexes and primary key constraints after loading data
def addIndexesAndConstraints(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
            ALTER TABLE {TableName} ADD CONSTRAINT {TableName}_pkey PRIMARY KEY (CensusTract);
            CREATE INDEX idx_{TableName}_State ON {TableName}(State);
        """)
        print("Added indexes and constraints.")
def disableIndexesAndConstraints(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"ALTER TABLE {TableName} DROP CONSTRAINT IF EXISTS {TableName}_pkey;")
        cursor.execute(f"DROP INDEX IF EXISTS idx_{TableName}_State;")
        print("Dropped indexes and Constraints .")
...
def setup_unlogged_table(conn):
    with conn.cursor() as cursor:
        # Drop the UNLOGGED table if it exists to ensure a clean start
        cursor.execute(f"DROP TABLE IF EXISTS {TempTableName}")
        # Create the UNLOGGED table anew
        cursor.execute(f"""
        ...
        """)
        print(f"UNLOGGED table {TempTableName} set up successfully")
        conn.commit()
...
def main():
    ...
    cmdlist = getSQLcmndsUnloggedTable(rlis)
    if CreateDB:
        createTable(conn)
    setup_unlogged_table(conn)
    disableIndexesAndConstraints(conn)
    load(conn, cmdlist)
    addIndexesAndConstraints(conn)
