...
# convert list of data rows into list of SQL 'INSERT INTO ...' commands
def getSQLcmndsTempTable(rowlist):
	cmdlist = []
	for row in rowlist:
		valstr = row2vals(row)
...
def setup_temp_table(conn):
    with conn.cursor() as cursor:
        # Drop the TEMPORARY table if it exists to ensure a clean start
        cursor.execute(f"DROP TABLE IF EXISTS {TempTableName}")
        cursor.execute("SET temp_buffers = '256MB';")
        # Create the TEMPORARY table anew
        cursor.execute(f"""
        ...
        """)
        print(f"TEMPORARY table {TempTableName} set up successfully")
        conn.commit()
...
def main():
    ...
    cmdlist = getSQLcmndsTempTable(rlis)
    if CreateDB:
        createTable(conn)
